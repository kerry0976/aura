...
# Created: 07/29/2020
# Editor: Kerry Sun
# Created to accommodate different timeframes between GPS and IMU

# MAT_FILENAME = 'test_GroveDemo12.mat'
...
def load(mat_filename):
    ...
    lon = flight_data.lon
    alt = flight_data.alt
    gnss_measuremnt = flight_data.GNSS_measurement
    ...
    while k < len(t):
        ...
        if t[k] == gnss_t[kGNSS]:
            ...
            gnss_measurement_pt = {
                'gnss_measurement': gnss_measuremnt[kGNSS],
                'time': float(gnss_t[kGNSS])
            }
            result['gnss_measument'].append(gnss_measurement_pt)
