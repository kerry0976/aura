...
void EKF15_mag::default_config()
{
    ...
    config.pseudo_range_SD = 0.2; // GPS pesudorange measurement noise std dev (m)
    config.range_rate_SD = 0.1;   // GPS pesudorange rate measurement noise std dev (m/s)
}
...
void EKF15_mag::measurement_update(IMUdata imu, GPSdata gps) {
    ...
    //Vector3d pos_gps(0, 0, 0);
    Vector3d pos_gps_ecef = lla2ecef(pos_gps); //loosely-EKF
    
    Vector3d pos_error_ecef = pos_gps_ecef - pos_ins_ecef;
    Vector3f pos_error_ned = ecef2ned(pos_error_ecef, pos_ref);
    cout <<  "what's going on ekf mag?" << endl;
